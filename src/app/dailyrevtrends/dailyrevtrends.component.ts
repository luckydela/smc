...
@Component({
  selector: 'app-dailyrevtrends',
  templateUrl: './dailyrevtrends.component.html',
  styleUrls: ['./dailyrevtrends.component.css']
})
export class DailyrevtrendsComponent implements OnInit {
  ...
  ngAfterViewInit(){
    this.service.getweeklyAnaytics(this.ud.email, this.ud.role).subscribe(data => {
...
drawCharts(labels, counts, amounts){
  ...
  if ($('#daily').length != 0 || $('#completedTasksChart').length != 0 || $('#websiteViewsChart').length != 0) {
    ...
    var dailySalesChart = new Chartist.Bar('#daily', dataDailySalesChart, optionsDailySalesChart);
    ...
    var completedTasksChart = new Chartist.Bar('#completed', dataCompletedTasksChart, optionsCompletedTasksChart);
